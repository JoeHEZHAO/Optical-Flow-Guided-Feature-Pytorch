"""
    ...
    4. Final loss is computed by first considering the consensus of all frames;
    5. The loss is average through all frames;
...
num_seg = 7
num_batch = 55 # Should be 128, if result not good, then try to run on server;
...
net = fine_tune_bninception_off_sobel(batch=num_batch, num_seg=num_seg).cuda()
net.modality_fuse = True
...
criterion = topk_crossEntrophy(top_k=0.1)
...
optimizer = optim.Adam(param, lr = 0.0001, betas= (0.9, 0.99), weight_decay=0.00005)
...
for epoch in range(10):
    for idx, (input, target) in enumerate(train_loader):
        ...
        if net.modality_fuse is False:
            ...
            rst1 = net.RGB_OFF_forward(input_var)
            target = target.cuda()
            ...
            print("Epoch {}, data batch {}, fuses loss is {}".format(epoch, idx, loss1.data[0].cpu()))
