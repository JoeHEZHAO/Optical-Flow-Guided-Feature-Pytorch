...
Declare: Code reference from http://data.lip6.fr/cadene/pretrainedmodels/;
    ...
    In this implementation:
        1. compute the spatial gradient by sobel_edges, which turns [b, h, w, c] into [b, h, w, c, 2];
        Then, we use reduce_mean() to merge x & y gradient; So, the spatial gradient branch is not trainable;
        2. compute the loss from consensus results; Consensus function is provided by TSN-Pytorch implementation:

    Notice: this is pytorch repo, there is no tf.image.sobel_edges() func exists;
    NOtice: Use the sobel filter in util.py file in /home/zhufl/videoPrediction/;
    Notice: There are two func we can use SobelFilter_Diagonal or SobelFilter
"""
...
class BNInception_OFF(nn.Module):
    ...
    def forward(self, input):
        ...
        #TODO: Set different return items for train and test
        if self.modality_fuse:
            ...
            return fc_action_motion_7, Feature_Generation_Score, fc_action_motion_14, conv2_relu_3x3_out
